...
if (user != "") {
  ...
  for (var i = 0; i < ca.length; i++) {
    ...
    if (c.indexOf("_ybizc0") == 0) {
      ...
      if (
        window.atob(userelcook[0].split("ybizc0=")[1]) == "BIZADMN" &&
        window.atob(userelcook[2]) == "valid"
      ) {
        $("#falqbody").hide();
  ...
  document.getElementById("imgcon").style.pointerEvents = "none";
  $("#qubitst,#backEditor,#editor").hide();
  $("#showandpost").show();
  document.getElementById("dosrmbt").style.margin = "0px";
...
function emdbqbt(label) {
  ...
  var newlk =
    '<iframe src="' +
    crlnk +
    '" style="width:100%;max-width:600px;height:500px;border:0px;" allowfullscreen></iframe>';
